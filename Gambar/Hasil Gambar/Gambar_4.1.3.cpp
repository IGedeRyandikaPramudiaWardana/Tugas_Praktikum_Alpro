...
void extractMessage() 
{
    ...
    char buff[256];
    int buffInt;
    char messageBinary[256 * 8 + 1]; // Ensure enough space for the binary message
    int messageIndex = 0;
    ...
    // Skip the BMP header (typically 54 bytes)
    fread(buff, 1, 54, flAsli);
    ...
    // Determine the actual binary length
    int binaryLen = strlen(messageBinary);

    // Calculate the number of characters to extract
    int extractedChars = binaryLen / 8;

    // Extracted message buffer
    char extractedMessage[extractedChars + 1];
