...
void extractMessageFromBMP(const char *inputFile, const char *outputFile) {
    ...
    // Salin bagian data gambar sebelum mengekstrak pesan
    ...
    for (int i = 0; i < messageLength * 8; i++) {
        ...
        // Ambil bit LSB dari byte
        extractedMessage[bitIndex / 8] |= (byte & 1) << (bitIndex % 8);
        bitIndex++;
    }

    // Tulis pesan ke file output
    fputs(extractedMessage, output);
    ...
    fclose(input);
    fclose(output);
    printf("Message extracted successfully from %s: %s\n", inputFile, extractedMessage);
}

int main() {
    const char *inputFile = "Yuta_with_message.bmp"; // Nama file gambar input
    const char *outputFile = "extracted_message.txt"; // Nama file output

    extractMessageFromBMP(inputFile, outputFile);
